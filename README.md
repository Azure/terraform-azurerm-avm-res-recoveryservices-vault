...
Description:   A map of private endpoints. The map key is the supplied input to var.private\_endpoints. The map value is the entire `azapi_resource` (`Microsoft.Network/privateEndpoints`) resource, including its `output` attribute with the ARM response body.
